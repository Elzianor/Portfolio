...
VS_OUTPUT VS_Main(VS_INPUT input)
{
    ...
    output.WorldViewPos = mul(input.Position, World).xyz;
...
float hex(float2 p)
{
    ...
    p.y += floor(p.x) * 0.5;
    p = abs(frac(p) - 0.5);
    return smoothstep(0.02, 0.05, abs(1.0 - max(p.x * 1.5 + p.y, p.y * 2.0)));
}
...
float4 PS_Main(VS_OUTPUT input) : SV_Target
{
    ...
    //float3 viewDirection = normalize(-input.WorldPos);
    ...
    //if (input.WorldPos.y > -0.01 && input.WorldPos.y < 0.01)
    //    color = 1.0;
